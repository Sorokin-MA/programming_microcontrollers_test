...
/**
 * Logs an error message to standard error and exits if a fault has occurred in
...
/**
 * @brief Cleans up the XML-RPC environment and shuts down the XML-RPC client
 * library.
 *
 ...
 * It ensures that all resources associated with these operations are properly
 * released.
 *
 ...
 * method on the server specified in SERVER_URI. It requires an initialized
 * XML-RPC environment and a valid EXPERIMENT_TOKEN to authenticate the request.
 *
...
void run_experiment(xmlrpc_env *env) {
  ...
  resultP = xmlrpc_client_call(env, SERVER_URI, "run_experiment", "(s)",
                               EXPERIMENT_TOKEN);
  dieIfFaultOccurred(env);
...
void set_current(xmlrpc_env *env, double current) {
  double signal;
  xmlrpc_client_call(env, SERVER_URI, "set_current", "(sd)", EXPERIMENT_TOKEN,
                     current);
  dieIfFaultOccurred(env);
}
...
double get_thermocouple_signal_mV(xmlrpc_env *env) {
  ...
  resultP = xmlrpc_client_call(env, SERVER_URI, "get_thermocouple_signal_mV",
                               "(s)", EXPERIMENT_TOKEN);
  dieIfFaultOccurred(env);
